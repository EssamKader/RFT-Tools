...
## 10. Dowel hook DIRECTION — outward from the column centroid, per bar (R10)

| Module / member | Status | Reason |
|---|---|---|
| `rft.core.footing_dowels.translate_dowel_bar_geometry` | ❌ Removed | Shift-only (no rotation), so every bar in a real array shared the SAME fixed `+X` hook direction regardless of its own position — found visibly wrong by Essam (a live-model screenshot showing a bar's hook bending back into the column). Its only caller (`footing_plan._build_dowel_plan`) now calls the replacement below; nothing else referenced it, so it was deleted rather than left dead. |
| `rft.core.footing_dowels.positioned_dowel_bar_geometry` (new) | ✅ New, replaces the above | Builds a bar's full geometry directly at its own `(u, v)` AND its own hook direction — `local_dowel_bar_geometry` (the single-representative-bar fallback) is now a thin call into this at the origin with the legacy `+X` direction, so its own already-tested output is pinned unchanged. |
| `rft.core.footing_dowels.dowel_outward_direction` (new) | ✅ New, pure | R10's own rule: corner bar → 45° diagonal (`normalize(sign(u), sign(v))`), face bar → straight outward perpendicular to its own face (derived from the bar's `(u, v)` against the column's own `half_u`/`half_v`, recomputed from `PerimeterLayout.bar_offset_mm` — `perimeter_bar_positions` itself does not return the BAR's own half-dimensions, only the TIE's different ones). |
| `rft.core.column_ties._outward_bisector` | ⚠️ Pattern reused, not the call | Same "outward = away from centroid, tested by dot product, flipped if it points inward" reasoning this repo already trusts for triangular tie construction — reused as a PATTERN for a rectangle's simpler, closed-form case, not imported (a different problem shape: three arbitrary points vs. an axis-aligned rectangle). `column_ties.py` is read-only here, untouched, per element isolation. |
| `rft.revit.footing_dowels._create_dowel_rebar`'s `norm` argument | ⚠️ Generalised, not reused as literal | Was a single hardcoded `XYZ.BasisY` (correct ONLY for the old fixed `+X` hook); now derived per bar (`_dowel_norm`, a 90-degree in-plane rotation of that bar's own hook vector) — the SAME #183 measurement (`norm` perpendicular to the bend's own plane) still governs, applied per bar instead of once. Reduces to exactly the old `XYZ.BasisY` for a `+X` hook, so the one live-host verification this repo has for a bent dowel's `norm` (#223) still covers that case. |

**Ruling — R10.** `docs/footing/spec-amendments.md` R10 records the finding and the rule, found by Essam from a live-model screenshot (not guessed, not a code review finding) — the exact gap `local_dowel_bar_geometry`'s own docstring had flagged since #202 ("flag to Essam before this runs against a live host if a specific hook direction... is intended instead").

**Verification status:** 12 new mock-object tests — `dowel_outward_direction` for all four corners (dot product with position is positive, i.e. genuinely outward) and all four faces (`tests/test_footing_dowels.py`), the composing module positions AND bends every bar correctly for both a corner-only fixture and a face-interior fixture (`tests/test_footing_plan.py`), and the Revit adapter's own per-bar `norm` varies across a real array while staying perpendicular to each bar's own hook (`tests/test_footing_revit_dowels.py`). Full suite: 1810 passed.

**✅ Verified on a live host, 2026-09-24** — see `docs/footing/verification/issue-232-dowel-hook-outward-direction.md`. The real 10-bar array (`build_footing_plan` run against column `425531`'s own live `Cw=300`/`Cd=600`/`Ccover=40`) was placed with the corrected direction-aware geometry, and every bar's hook-far-end was independently confirmed FARTHER from the column's own centroid than its own bend corner (genuinely outward, not toward it) — `badDirection=0` across corner, b-face and h-face bars, all present in this one real array. Position remained correct too (`badPosition=0`). Superseded the earlier `issue-222-real-dowel-array-inside-column.md` array, which had verified position only and still used the old fixed-direction placeholder for the hook itself (that doc's own addendum records this).

## 11. Modeless window infrastructure (`specs/ui-single-window.md` pattern) — audited by #205

| Module / member | Status | Reason |
|---|---|---|
| `rft.ui.shared_styles.window_xaml`/`resolve_shared_styles` | ✅ Reused as-is | Fully generic (pure text/paths, no Revit/WPF import) — `FootingWindow.xaml` carries the exact same `Source="SharedStyles.xaml"` placeholder ColumnRFT's own window uses, rewritten to the real palette location before parse. No fork needed. |
| `rft.ui.inputs.parse_positive_float`/`parse_optional_positive_int` | ✅ Reused as-is | Generic numeric TextBox parsing (blank-vs-zero handling, a named field in every error message) — used unchanged for the mesh/dowel shared inputs. The rest of `inputs.py` (closure types, layer options, H_avail) is beam-specific and not imported. |
| `rft.ui.persistence` (to_store/from_store SHAPE) | ⚠️ Pattern reused, not the module | New `rft.ui.footing_persistence` mirrors the shape (`to_store`/`from_store`, `script.store_data`/`load_data` call pattern, `UniqueId`-keyed type fields) with this tool's OWN field names — `persistence.py`'s own `TEXT_FIELDS`/`TYPE_FIELDS`/`SETTINGS_SLOT` constants are hardcoded to SimpleBeamRFT, per that module's own docstring. No "request-constituting fields withheld" rule applies here (unlike beam's U10) — see `footing_persistence.py`'s own docstring for why. |
| `rft.ui.report` | ❌ Not reused | Confirmed beam-specific (`derivation_lines`/`main_bars_lines`/etc., all beam `ReportInputs`-shaped) by direct inspection — not imported by ColumnRFT either, which built its own `rft.core.column_report` instead. |
| `rft.core.column_report`'s `ReportSection`/`render`/`batch_*_section` shape | ⚠️ Pattern reused, not the module | New `rft.core.footing_report` mirrors the shape exactly (pure, `(heading, lines)` sections, one `render`) with footing/batch-shaped section builders reading `FootingGeometry`/`DowelColumnSection`/`FootingPlan`/`footing_batch.BatchPlan` objects instead of column ones. `column_report.py` is read-only here, untouched. |
| ColumnRFT's own `_dispatch_to_revit_context`/`_run_in_revit_context` wrapper around `revit.events.execute_in_revit_context` | ⚠️ Pattern reused, not shared code | Confirmed NOT a shared library function — ColumnRFT and SimpleBeamRFT each carry their own independent copy of the same wrapper. `FootingWindow` carries a third, following the identical shape (refuse `FootingHostError`/`ColumnHostError` as an expected outcome with `forms.alert`, catch-all else as a failure, reset the in-flight flag in `finally`). |
| ColumnWindow.xaml's `TabItem` `ControlTemplate` | ✅ Reused verbatim (shape, not a shared Style) | Copied into `FootingWindow.xaml` unchanged — brushes come from the shared palette so a colour change repaints both, but a WPF `Style` is per-window by construction (`ColumnWindow.xaml`'s own comment, `tests/test_ui_shared_styles.py`). |

**Scope decision, not a code-reuse verdict:** top mesh's U/L-shape toggle, `dowel_tie` diameter/spacing, and `perimeter_tie` diameter/spacing/quantity are named by #205's own ticket text but have no Revit placement adapter anywhere in this repo — the window does not expose inputs for them (`IsolatedFooting.extension/CONTEXT.md`'s own #205 scope note explains why: exposing an input this tool cannot act on is guessed scope, not wiring).

**Verification status:** mock-object tests for the two new pure modules — `tests/test_footing_persistence.py` (round-trip, plain-data-only, footing/column-scoped fields never persisted), `tests/test_footing_report.py` (every section, `render`). `tests/test_xaml_parses_as_xml.py` and `tests/test_column_xaml.py`'s generic guards (well-formed XML, no double-hyphen in a comment, every resource key resolvable) now cover `FootingWindow.xaml` too, via `tests/xaml_keys.py`'s `WINDOW_XAML_PATHS`. **Not independently WPF-tested** — nothing in this repo can execute WPF under plain CPython (the standing constraint every XAML guard in this repo states); the window's own live-host verification is a human step (pressing the button in Revit), per `specs/ui-single-window.md`'s own "Definition of done."
