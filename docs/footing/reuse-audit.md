...
**Verification status:** mock-object tests prove grouping (`tests/test_footing_batch.py`), the ownership module (`tests/test_footing_ownership.py`), and the full orchestration — footing-type filter, column-type-mismatch exclusion, every #220/#221/#228 refusal propagated, survivors-only grouping, one-transaction-all-or-nothing, mid-batch rollback restoring an already-deleted candidate's own bar, replacement counted-then-deleted exactly (`tests/test_footing_revit_batch.py`). **Not yet verified against a live host** — a live-host tracer bullet (mirroring #223's own N-bar-loop proof, but for N FOOTINGS in one transaction) is this ticket's own outstanding step; see whichever verification doc lands next for this ticket, or its absence if none has yet.
