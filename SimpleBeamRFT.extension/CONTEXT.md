...
Element-specific. The repo-wide rules that govern **every** element live in the
root [`CONTEXT.md`](../CONTEXT.md); this file holds only what is true of the
beam tool and of nothing else. Moved here from the root file per
[`docs/token-efficient-expansion.md`](../docs/token-efficient-expansion.md) §2 —
the root file is loaded at the start of every session regardless of which
element is being worked on, so beam-only rules sitting in it were a cost paid
on every column session too.
